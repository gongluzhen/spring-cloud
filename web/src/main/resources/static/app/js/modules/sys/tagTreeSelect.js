var tree, key, lastValue, nodeList, url, type, setting;
$(document).ready(function(){
	lastValue = "";
	nodeList = [];
	url = $("#url").val();
	type = getQueryString("type", url);
	setting = {
		view: {
			selectedMulti: false,
	...
	var u = ctx + url + (url.indexOf('?') == -1 ? '?' : '&') + '&extId=' + $("#extId").val() + '&t=' + new Date().getTime();
	$.get(u, function(zNodes){
		// 初始化树结构

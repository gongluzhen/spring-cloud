	...
	parseDate : function(format, date, newformat, opts) {
		...
		if(date && date != null) {
			if( !isNaN( date - 0 ) && String(format).toLowerCase() === "u") {
				...
				timestamp = new Date( parseFloat(date) ); // sapit 修正日期格式化错误问题
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatter = function (rowId, cellval , colpos, rwdat, _act, irow){ // sapit add irow 参数
			var cm = ts.p.colModel[colpos],v;
			if(cm.formatter !== undefined) {
				rowId = String(ts.p.idPrefix) !== "" ? $.jgrid.stripPref(ts.p.idPrefix, rowId) : rowId;
				//var opts= {rowId: rowId, colModel:cm, gid:ts.p.id, pos:colpos };
				var opts= {rowId: rowId, colModel:cm, gid:ts.p.id, pos:colpos, irow:irow }; // sapit add irow 参数
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			var v,prp;
			v = formatter(rowId,cell,pos,srvr,'add',irow); // sapit add irow
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join('')); // sapit 恢复3.7被注释的代码，下面这一行不支持ie8
					//ts.firstElementChild.innerHTML += rowData.join(''); // append to innerHTML of tbody which contains the first row (.jqgfirstrow)
					ts.grid.cols = ts.rows[0].cells; // update cached first row
				}
			}
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$("#gbox_"+$.jgrid.jqID($t.p.id)).css("width",(nwidth+2)+"px"); // sapit 修正宽度计算错误
	...
	setGridHeight : function (nh) {
		return this.each(function (){
			...
			if($t.p.frozenColumns === true){
				...
				$('#'+$.jgrid.jqID($t.p.id)+"_frozen").parent().height(bDiv.height() - 16+($.jgrid.msie?0:4)); // sapit 修正IE下高度计算错误
			}
			$t.p.height = nh;
...
// Modal functions
	...
	hibusinessIntegrationdal : function (selector,o) {
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		$("a.ui-jqdialog-titlebar-close",mh).click(function(){
			...
			self.hibusinessIntegrationdal("#"+$.jgrid.jqID(aIDs.themodal),{gb:gboxclose,jqm:p.jqModal,onClose:oncm, removemodal: p.removemodal || false, formprop : !p.recreateForm || false, form: p.form || ''});
			return false;
		});
		...
		if(p.closeOnEscape === true){
			$(mw).keydown( function( e ) {
				if( e.which === 27 ) {
					...
					self.hibusinessIntegrationdal("#"+$.jgrid.jqID(aIDs.themodal),{gb:p.gbox,jqm:p.jqModal,onClose: cone, removemodal: p.removemodal || false, formprop : !p.recreateForm || false, form: p.form || ''});
				}
			});
		}
	},
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		try {
			if($("#info_dialog").attr("aria-hidden") === "false") {
				$.jgrid.hibusinessIntegrationdal("#info_dialog",{jqm:jm});
			}
			$("#info_dialog").remove();
		} catch (e){}
		$.jgrid.createModal({
		...
		$("#closedialog", "#info_id").click(function(){
			self.hibusinessIntegrationdal("#info_dialog",{
				jqm:jm,
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			case "select" :
				...
				$(elem).css({width:"98%"}); // sapit select attr add width
				var msl, ovm = [];
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height2:'+hth+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>'); // sapit 修正标题栏高度计算错误
				...
				if($t.p.footerrow) {
					...
					$t.grid.fsDiv = $('<div style="position:absolute;left:0px;top2:'+(parseInt(top,10)+parseInt(hth,10) + parseInt(hbd,10)+1)+'px;" class="frozen-sdiv ui-jqgrid-sdiv"></div>'); // sapit 修正锁定列，统计行位置计算错误
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
					...
					$($t.grid.fhDiv).height($($t.grid.hDiv).height()); // sapit add 修正锁定列，标题栏高度计算错误
					//$($t.grid.fbDiv).height($($t.grid.bDiv).height()-16);
					$($t.grid.fbDiv).height($($t.grid.bDiv).height()-16+($.jgrid.msie?0:4)); // sapit 修正IE下高度计算错误
					$($t.grid.fsDiv).css('top', $($t.grid.hDiv).height() + $($t.grid.fbDiv).height() - $($t.grid.fsDiv).height() - 1); // sapit 修正锁定列，统计行位置计算错误
					// sapit 凑合用IE8下冻结列显示问题
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#"+fid+"_search").bind('click', function(){
					...
					if(p.closeAfterSearch) {
						$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID($t.p.id),jqm:p.jqModal,onClose: p.onClose});
					}
					return false;
				});
		...
		return this.each(function(){
							...
							if(ret[0] === false) {
								...
								if(postdata[oper] === opers.addoper ) {
									...
									if(rp_ge[$t.p.id].closeAfterAdd) {
										...
										$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal,onClose: rp_ge[$t.p.id].onClose, removemodal: rp_ge[$t.p.id].removemodal, formprop: !rp_ge[$t.p.id].recreateForm, form: rp_ge[$t.p.id].form});
									...
									if(rp_ge[$t.p.id].closeAfterEdit) {$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal,onClose: rp_ge[$t.p.id].onClose, removemodal: rp_ge[$t.p.id].removemodal, formprop: !rp_ge[$t.p.id].recreateForm, form: rp_ge[$t.p.id].form});}
								}
			...
			$("#"+$.jgrid.jqID(IDs.themodal)).keydown( function( e ) {
				...
				if(e.which === 27) {
					...
					if(cle)	{$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:p.gbox,jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose, removemodal: rp_ge[$t.p.id].removemodal, formprop: !rp_ge[$t.p.id].recreateForm, form: rp_ge[$t.p.id].form});}
					return false;
				}
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			$("#"+IDs.themodal).keydown( function( e ) {
				if(e.which === 27) {
					if(rp_ge[$t.p.id].closeOnEscape) {$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:p.gbox,jqm:p.jqModal, onClose: p.onClose, removemodal: rp_ge[$t.p.id].removemodal, formprop: !rp_ge[$t.p.id].recreateForm, form: rp_ge[$t.p.id].form});}
					return false;
				}
				if(p.navkeys[0]===true) {
			...
			focusaref();
			$("#cData", "#"+frmtb+"_2").click(function(){
				$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: p.onClose, removemodal: rp_ge[$t.p.id].removemodal, formprop: !rp_ge[$t.p.id].recreateForm, form: rp_ge[$t.p.id].form});
				return false;
			});
			$("#nData", "#"+frmtb+"_2").click(function(){
				$("#FormError","#"+frmtb).hide();
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						var ajaxOptions = $.extend({
							...
							complete:function(data,status){
								...
								rp_ge[$t.p.id].processing=false;
								if(ret[0]) {$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose});}
							}
						...
						if (ret[0]) {
							if (rp_ge[$t.p.id].useDataProxy) {
								...
								if(dpret[0] === false ) {
									...
									$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:p.jqModal, onClose: rp_ge[$t.p.id].onClose});
								}
							}
							else {
								if(ajaxOptions.url === "clientArray") {
				...
				$("#eData", "#"+dtbl+"_2").click(function(){
					$.jgrid.hibusinessIntegrationdal("#"+$.jgrid.jqID(IDs.themodal),{gb:"#gbox_"+$.jgrid.jqID(gID),jqm:rp_ge[$t.p.id].jqModal, onClose: rp_ge[$t.p.id].onClose});
					return false;
				});
				if(onBeforeShow) {rp_ge[$t.p.id].beforeShowForm.call($t,$("#"+dtbl));}
...
$.jgrid.extend({
	...
	collapseNode : function(rc) {
		return this.each(function(){
			...
			// sapit add eveal onCollapseNode

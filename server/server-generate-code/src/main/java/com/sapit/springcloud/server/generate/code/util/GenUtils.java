package com.sapit.springcloud.server.generate.code.util;
...
import com.sapit.springcloud.common.util.FreeMarkers;
import com.sapit.springcloud.common.util.PropertiesUtil;
import com.sapit.springcloud.common.util.StringUtils;
import com.sapit.springcloud.common.util.mapper.JaxbMapper;
import com.sapit.springcloud.moudle.generate.code.GenCategory;
...
public class GenUtils {
	...
	public static String getTemplatePath() {
		try {
			...
			if (file != null) {
				return file.getAbsolutePath() + File.separator + StringUtils.replaceEach(GenUtils.class.getName(), new String[] { "util." + GenUtils.class.getSimpleName(), "." },
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme) {
		...
		model.put("projectPath", StringUtils.lowerCase(genScheme.getProjectPath()));
		model.put("basePackageName", StringUtils.lowerCase(genScheme.getBasePackageName()));
		model.put("moduleName", StringUtils.lowerCase(genScheme.getModuleName())); //aa.bb
		model.put("moduleProjectName", StringUtils.lowerCase(genScheme.getModuleName()).replaceAll("\\.", "-")); //aa-bb
		model.put("moduleWebName", StringUtils.lowerCase(genScheme.getModuleName()).replaceAll("\\.", "/")); //aa/bb
		model.put("modulePermissionsName", StringUtils.lowerCase(genScheme.getModuleName()).replaceAll("\\.", ":")); //aa:bb
		...
		model.put("functionAuthor", StringUtils.isNotBlank(genScheme.getFunctionAuthor()) ? genScheme.getFunctionAuthor()
				: (genScheme.getCurrentLoginUser() == null ? "" : genScheme.getCurrentLoginUser().getName()));
		model.put("functionVersion", DateUtils.getDate());

		model.put("dbType", PropertiesUtil.dbName);
	 ...
	 * @return
	 */
	public static String generateToFile(GenTemplate tpl, Map<String, Object> model, boolean isReplaceFile, String projectPath) {
		// 获取生成文件
		String fileName = projectPath + File.separator + StringUtils.replaceEach(FreeMarkers.renderString(tpl.getFilePath() + "/", model),
				new String[] { "//", "/", "." }, new String[] { File.separator, File.separator, File.separator }) + FreeMarkers.renderString(tpl.getFileName(), model);
